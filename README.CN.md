...
# 1. 安装依赖
npm ci
cp .env.example .env            # 填入 OPENROUTER_API_KEY，默认走 OpenRouter Free mode
...
- **LLM**：默认使用 OpenRouter Free mode，并在 `config.yaml` 中显式配置为 `type: openai` 的 OpenAI-compatible provider。
  在 `.env` 中填写 `OPENROUTER_API_KEY`，保留 `OPENROUTER_BASE_URL=https://openrouter.ai/api/v1`，模型默认使用 `qwen/qwen3-coder:free` 这类免费模型 slug。
