...
import { runExecute, type ExecuteResult } from './execute.js';
...
export async function runBootstrap(opts: BootstrapOptions): Promise<BootstrapResult> {
  ...
  console.log(chalk.green('✔'), t().bootstrap.worktreeReady(prepared.worktree, prepared.branch));
  ...
  console.log(chalk.cyan('→'), t().bootstrap.compileStarted);
  let compiled;
  try {
    ...
    console.error(chalk.red('✖'), t().bootstrap.compileFailed(code, message));
    return finishBootstrap(prepared, 'compile-failed', [], [], opts);
  }

  if (!compiled.planPath) {
    console.log(chalk.yellow('!'), t().bootstrap.compileCancelled);
    return finishBootstrap(prepared, 'cancelled', [], [], opts);
  }

  console.log(chalk.cyan('→'), t().bootstrap.executeStarted);
  ...
  if (execution.status !== 'ok') {
    console.error(chalk.red('✖'), t().bootstrap.executeFailed(execution.status));
  ...
  console.log(chalk.cyan('→'), t().bootstrap.qualificationStarted);
  if (opts.dockerQualification) {
    console.warn(chalk.yellow('!'), t().bootstrap.qualificationDockerExperimental);
  }
  ...
  for (const check of checks) {
    ...
    console.log(check.ok ? chalk.green('✔') : check.required ? chalk.red('✖') : chalk.yellow('!'), message);
  }
  ...
  if (!qualified) {
    console.error(chalk.red('✖'), t().bootstrap.promotionBlocked);
  ...
  if (opts.promote) {
    ...
    status = 'promoted';
    console.log(chalk.green('✔'), t().bootstrap.promoted(prepared.branch));
  } else {
    console.log(chalk.green('✔'), t().bootstrap.candidateReady(prepared.branch));
  }
  return finishBootstrap(
    { ...prepared, candidateCommit },
    status,
...
async function finishBootstrap(
  ...
  console.log(chalk.green('✔'), t().bootstrap.reportWritten(reportPath));
  if (opts.cleanup) {
    await simpleGit({ baseDir: prepared.repository }).raw(['worktree', 'remove', '--force', prepared.worktree]);
    console.log(chalk.green('✔'), t().bootstrap.cleanupDone(prepared.worktree));
  }
  return result;
}
